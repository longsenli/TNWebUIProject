	...
	$.ajax({
		...
		success: function(dataRes) {
			...
			if(dataRes.status == 1) { 
				...
				batteryGearMarkProductionProcessSlctFun();
			} else {
...
function batteryGearMarkProductionProcessSlctFun() {
	$.ajax({
...
function getOnRackRecord(selectType) {
	...
	$.ajax({
		url: window.serviceIP + "/api/chargepack/getbatteryGearMark",
		type: "POST",
		data: formData,
		processData: false,
		contentType: false,
...
function closebatteryGearMarkModel(modelName) {
	$("#" + modelName).modal('hide');
}

function setPutOnNum() {
	var num = parseInt($("#batteryGearMarkAddForm" + " #materialname").find("option:selected").text().split("###")[1].trim());
	$("#batteryGearMarkAddForm" + " #productionnumber").val(num);
	$("#batteryGearMarkAddForm" + " #realnumber").val(num);
}

function addbatteryGearMark() {
	$('#batteryGearMarkAddForm #lineid').selectpicker('refresh');
	$('#batteryGearMarkAddForm #lineid').selectpicker('render'); 

	$('#batteryGearMarkAddForm #worklocation').selectpicker('refresh');
	$('#batteryGearMarkAddForm #worklocation').selectpicker('render'); 

	$('#batteryGearMarkAddForm #materialname').selectpicker('refresh');
	$('#batteryGearMarkAddForm #materialname').selectpicker('render'); 

	$('#batteryGearMarkAddForm #materialtype').selectpicker('refresh');
	$('#batteryGearMarkAddForm #materialtype').selectpicker('render'); 
	...
	setPutOnNum();
	$("#batteryGearMarkAddForm" + " #plantid").val(document.PlantToLineSelectForm.industrialPlantSlct.value.toString());
	$("#batteryGearMarkAddForm" + " #processid").val(document.PlantToLineSelectForm.productionProcessSlct.value.toString());
	$("#batteryGearMarkAddForm" + " #staffid").val(localStorage.userID);
	$("#batteryGearMarkAddForm" + " #staffname").val(localStorage.username);
	var today = new Date();
	$("#batteryGearMarkAddForm" + " #putondate").val(today.format("yyyy-MM-dd"));

	$("#myAddModal").modal('show');
}

function repairbatteryGearMark() {
	...
	//console.log(row[0]);
	$("#batteryGearMarkRepairForm" + " #id").val(row[0].id);
	$("#batteryGearMarkRepairForm" + " #realnumber").val(row[0].realnumber);
	$("#batteryGearMarkRepairForm" + " #repairnumber").val(row[0].repairnumber);
	$("#batteryGearMarkRepairForm" + " #repaircombine").val(row[0].repaircombine);
	$("#batteryGearMarkRepairForm" + " #remark").val(row[0].remark);
	//	if(row[0].repaircombine) {
	//		if(row[0].repaircombine.length > 50) {
	//			$("#batteryGearMarkRepairForm" + " #repaircombine").height((row[0].remark.length % 50) * 20);
	//		}
	//	}

	$("#batteryGearMarkRepairForm" + " #repairid").val(localStorage.userID);
	$("#batteryGearMarkRepairForm" + " #repairname").val(localStorage.username);
	var today = new Date();
	$("#batteryGearMarkRepairForm" + " #repairtime").val(today.format("yyyy-MM-dd hh:mm"));
...
function pullOffbatteryGearMark() {
	...
	$.ajax({
		url: window.serviceIP + "/api/chargepack/pulloffbatteryGearMark",
		type: "POST",
		contentType: "application/json",
		dataType: "json",
...
function deletebatteryGearMark() {
	var row = $.map($('#table').bootstrapTable('getSelections'), function(row) {
		return row;
	});
	if(row.length != 1) {
	...
	$.ajax({
		url: window.serviceIP + "/api/chargepack/detetebatteryGearMark?id=" + row[0].id,
		type: "POST",
		contentType: "application/json",
		dataType: "json",
...
function savebatteryGearMarkModel(modelID, formID) {
	...
	if(formMap.hasOwnProperty("materialname")) {
		...
		formMap["materialid"] = $("#batteryGearMarkAddForm" + " #materialname").val();
		formMap["materialname"] = $("#batteryGearMarkAddForm" + " #materialname").find("option:selected").text().split("###")[0].trim()
	}

	if(formID == 'batteryGearMarkRepairForm') {
	...
	$.ajax({
		url: window.serviceIP + "/api/chargepack/changebatteryGearMark",
		type: "POST",
		contentType: "application/json",
		dataType: "json",
...
//						<button type="button" class="btn btn-default" onclick="savebatteryGearMarkModel()">保存 </button>
//						<button type="button" class="btn btn-default" onclick="closebatteryGearMarkModel()">关闭 </button>
//					</div>
//				</div>
//				<!-- /.modal-content -->
//			</div>
