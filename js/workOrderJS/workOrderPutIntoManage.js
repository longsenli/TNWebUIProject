function dailyProductionConfirmIndustrialPlantSlctFun() {
	$.ajax({
		...
		success: function(dataRes) {
			...
			if(dataRes.status == 1) { 
				...
				// 	$('#industrialPlantSlct').selectpicker('hide');
				// }
				dailyProductionConfirmProcessSlctFun();
			} else {
...
function dailyProductionConfirmProcessSlctFun() {
	$.ajax({
...
function dailyProductionConfirmLineSlctFun() {
	//	return true;
...
function dailyProductionConfirmWorkingLocationSlctFun() {
	var formData = new FormData();
	formData.append("plantID", document.PlantToLineSelectForm.industrialPlantSlct.value.toString());
	formData.append("processID", document.PlantToLineSelectForm.productionProcessSlct.value.toString());
...
var timeNum = 1;
var selected = false;
function workOrderPutIntoManage() {
	var columnsArray = [];
	// columnsArray.push({
	// 	checkbox: true,
	...
	formData.append("endTime", document.getElementById("endTime").value.toString());
	selected = false;
	$.ajax({
		url: window.serviceIP + "/api/order/workOrderPutIntoManage",
		type: "POST",
		data: formData,
		processData: false,
		contentType: false,
					...
					onClickRow: function(row, element) {
						if(row.信息 == '总计') {
							...
							if(selected) {
								...
								selected = true;
								$("#table").bootstrapTable('resetSearch', row.物料型号);
